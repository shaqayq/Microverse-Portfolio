...
function CreateWorkSection(key) {
  ...
  for(let j in projects[key].jobdescription){
  ...
  for(let t in projects[key].technologies){
    const element=document.createElement('li');
...
element.addEventListener('click', function (){
  ...
  for(let j in projects[btnID].jobdescription){
    const element=document.createElement('li');
 ...
 right.appendChild(left);
  ...
  for(let t in projects[btnID].technologies){
    const element=document.createElement('li');
    tech.appendChild(element);
...
const cancel = document.querySelector('.pop-close');

cancel.addEventListener('click' , () => {
  alert('hi');
    document.querySelector('.popup').style.visibility ='hidden';
    document.querySelector('#blur').classList.remove('pop-background');
